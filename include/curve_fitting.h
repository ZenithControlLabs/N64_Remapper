/*
  ...
  Refactored for use in the codebase (use float instead of double)
*/
...
void cpyArray(float *src, float*dest, int n){
  for (int i = 0; i < n*n; i++){
    dest[i] = src[i];
  }
}
...
//Raise x to power
float curveFitPower(float base, int exponent){
  if (exponent == 0){
    return 1;
  } else {
    float val = base;
...
/*int fitCurve (int order, int nPoints, float py[], int nCoeffs, float *coeffs) {
  ...
  int i, j;
  float T[MAX_ORDER] = {0}; //Values to generate RHS of linear equation
  float S[MAX_ORDER*2+1] = {0}; //Values for LHS and RHS of linear equation
  float denom; //denominator for Cramer's rule, determinant of LHS linear equation
  float x, y;

  float px[nPoints]; //Generate X values, from 0 to n
  for (i=0; i<nPoints; i++){
	px[i] = i;
  }
  ...
  float masterMat[nCoeffs*nCoeffs]; //Master matrix LHS of linear equation
  ...
  float mat[nCoeffs*nCoeffs]; //Temp matrix as det() method alters the matrix given
