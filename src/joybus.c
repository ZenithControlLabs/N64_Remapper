...
static void __not_in_flash_func(rx_fifo_not_empty_handler)(void)
{
    ...
    if (packet == PROBE || packet == RESET) {
        ...
        // don't care. it's not a valid packet, we're not going to treat it as such
        return;
    }

    
    _receive_end = make_timeout_time_us(4);
    irq_set_enabled(PIO0_IRQ_0, false);  // disable RX interrupts. we do not care what happens on the bus now since we have control.
    ...
    _len = len;
    irq_set_enabled(PIO0_IRQ_1, true);  // ok now enable TX interrupts, we need to fill the TX queue
...
static void __not_in_flash_func(tx_fifo_empty_handler)(void) {
    ...
    if (_ind == _len) {
        // we are officially done transferring, since there is nothing left in our queue (software side)
        _ind = 0;
...
void joybus_init_comms(void) {
    ...
    irq_set_enabled(PIO0_IRQ_0, true); // we want to be interrupted as soon as new byte is ready
