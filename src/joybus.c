...
static void __not_in_flash_func(rx_fifo_not_empty_handler)(void) {
  ...
  irq_set_enabled(PIO0_IRQ_0,
                  false); // disable RX interrupts. we do not care what happens
                          // on the bus now since we have control.
...
static void __not_in_flash_func(tx_fifo_empty_handler)(void) {
  ...
  if (_ind == _len) {
    // we are officially done transferring, since there is nothing left in our
    // queue (software side)
    _ind = 0;
